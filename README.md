...
The latest **joint denoising and 2× reconstruction** study trained on
[59,992 reused examples](docs/joint-reuse-training.md). Training finished at
**69 epochs** through early stopping; **epoch 59 is the selected best**. Native
validation averages 29.82 dB PSNR / 0.9173 SSIM, compared with 25.98 dB / 0.8768
...
![Epoch-59 comparison: noisy input, a-trous denoiser, high-sample reference, AI reconstruction, AI followed by denoising, and denoising followed by AI](ml/reports/figures/joint-epoch59-corridor-64spp.png)
...
The bottom row compares AI alone, AI then denoising, and denoising then AI.*

![Training loss, native image quality and HDR error through 69 epochs](ml/reports/figures/joint-training-history.png)

See the **[three-scene gallery, sample-count chart and exact results](ml/reports/joint-reconstruction-results.md)**.
These are validation results from one seed. The bundled viewer model remains the
older diffuse pilot below; the new checkpoint has not been promoted to a release.
...
It currently has no image texture loader or participating media. The released
reconstruction model has a restricted diffuse training domain; the experimental
joint model's broader transport domain remains unqualified. Caustic maps have finite-radius bias, and approximate
